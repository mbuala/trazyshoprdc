...
$dest_dossier = 'images/foire/profil/'; //nom du dossier ou vous allez stocké vos images
$dest_fichier="";

//*******************²*******************
if(!$getimagesize = getimagesize($_FILES['profil_img']['tmp_name'])) {
$erreurs[] = "Le fichier n'est pas une image valide.";
}
//*******************************
else {
if( (!in_array( get_extension2($_FILES['profil_img']['name']), $extensions_ok ))or (!in_array($getimagesize[2], $typeimages_ok )))
{
...
if( file_exists($_FILES['profil_img']['tmp_name']) and filesize($_FILES['profil_img']['tmp_name']) > $taille_max)
{
$erreurs[] = "Votre fichier doit faire moins de $taille_ko Ko !";
}
else{

$dest_fichier = basename($_FILES['profil_img']['name']);
...
//********************************
if(move_uploaded_file($_FILES['profil_img']['tmp_name'], $dossier . $dest_fichier)) {
