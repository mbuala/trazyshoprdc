...
$dest_dossier = 'images/foire/img_fond/'; //nom du dossier ou vous allez stocké vos images
$dest_fichier="";

//*******************²*******************
if(!$getimagesize = getimagesize($_FILES['couverture_img']['tmp_name'])) {
$erreurs[] = "Le fichier n'est pas une image valide.";
}
//*******************************
else {
if( (!in_array( get_extension3($_FILES['couverture_img']['name']), $extensions_ok ))or (!in_array($getimagesize[2], $typeimages_ok )))
{
...
if( file_exists($_FILES['couverture_img']['tmp_name']) and filesize($_FILES['couverture_img']['tmp_name']) > $taille_max)
{
$erreurs[] = "Votre fichier doit faire moins de $taille_ko Ko !";
}
else{

$dest_fichier = basename($_FILES['couverture_img']['name']);
...
//********************************
if(move_uploaded_file($_FILES['couverture_img']['tmp_name'], $dossier . $dest_fichier)) {
...
if(@$erreurs[0]!=""){
print("<div class=\"erreurFormulaire\"><div class=\"erreurEntete\"> un probleme est survenu lors de l'upload de l'image</div><div class=\"erreurMessage\"> ");
